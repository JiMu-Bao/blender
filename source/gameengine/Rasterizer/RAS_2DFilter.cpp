...
static std::string predefinedUniformsName[RAS_2DFilter::MAX_PREDEFINED_UNIFORM_TYPE] = {
	...
	"bgl_RenderedTextures[0]", // RENDERED_TEXTURES_UNIFORM
...
void RAS_2DFilter::BindTextures(RAS_OffScreen *depthofs, RAS_OffScreen *colorofs)
{
	...
	if (m_predefinedUniforms[RENDERED_TEXTURES_UNIFORM] != -1) {
		for (unsigned short i = 0, slots = colorofs->GetNumColorSlot(); i < slots; ++i) {
...
void RAS_2DFilter::BindUniforms(RAS_ICanvas *canvas)
{
	...
	if (m_predefinedUniforms[RENDERED_TEXTURES_UNIFORM] != -1) {
		static const int units[] = {9, 10, 11, 12, 13, 14, 15, 16};
		SetUniformiv(m_predefinedUniforms[RENDERED_TEXTURES_UNIFORM], RAS_Uniform::UNI_INT, units, sizeof(int) * 8, 8);
	}
	if (m_predefinedUniforms[DEPTH_TEXTURE_UNIFORM] != -1) {
